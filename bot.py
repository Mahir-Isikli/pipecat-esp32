...
from tools import tools, register_object_tracking_functions
...
async def run_bot(transport: BaseTransport, runner_args: RunnerArguments):
    ...
    tts = CartesiaTTSService(
        ...
        voice_id="3f017cdf-4095-4443-9ed3-eb8171a7952a",  # British Reading Lady
    )

    llm = OpenAILLMService(api_key=os.getenv("OPENAI_API_KEY"), model="gpt-4.1")

    # Register object tracking functions
    register_object_tracking_functions(llm)

    messages = [
        {
            "role": "system",
            "content": "You are a friendly AI assistant. Respond naturally and keep your answers conversational. Be aware that everything you say is being read out loud. You are an object tracking assistant that can monitor detected objects from live tracking data. You can check object counts by calling the check_object_counts function and update quantities using the update_object_count function. When users tell you about changes in object counts, use the update function to adjust the tracking data. Keep your style good for a voice assistant. Answer questions concisely and don't suggest whether the user needs help with anything else.",
        },
    ]
